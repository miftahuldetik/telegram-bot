...
# ----- Konfigurasi Bot -----
TOKEN = "7237229468:AAH0QEea9LQkP5XTLNimnI6cPE1AhWrxBmQ"  # Token Bot Telegram
GAS_WEBAPP_URL = "https://script.google.com/macros/s/AKfycbwNXyltn57HV601RA18pLFIfJKuXIIMPHysecc8J4xSvnb7JlASLrvhAmP5F6MrjhJm/exec"  # URL Web App GAS
...
(
    ...
    PILIH_URGENSI,
    PILIH_REQUEST_TYPE,
    PILIH_CONTENT,      # Jika request type ≠ Business Intelligence
    MINTA_MO_NUMBERS,   # Jika content type adalah ADV/CPD atau Specific MO
    MINTA_TGL_MULAI,
    MINTA_TGL_SELESAI,
    MINTA_BRAND,
    MINTA_AGENCY,
    MINTA_ATTACHMENTS,
    MINTA_CC_EMAIL,     # Meminta ccEmail
    PILIH_LEGAL_CHECK,
    KONFIRMASI
) = range(14)
...
def minta_brief(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Pilih tingkat Urgensi:", reply_markup=reply_markup)
...
def tombol_urgensi(update: Update, context: CallbackContext) -> int:
    ...
    keyboard = [
        [InlineKeyboardButton("Update Data", callback_data="reqtype_Update Data")],
        [InlineKeyboardButton("Campaign Analysis", callback_data="reqtype_Campaign Analysis")],
        [InlineKeyboardButton("Business Intelligence", callback_data="reqtype_Business Intelligence")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def tombol_request_type(update: Update, context: CallbackContext) -> int:
    ...
    if reqtype == "Business Intelligence":
        query.message.reply_text("Masukkan tanggal mulai (format: YYYY-MM-DD):")
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("Microsite", callback_data="content_Microsite")],
            [InlineKeyboardButton("Specific MO", callback_data="content_Specific MO")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        query.message.reply_text("Pilih Content Type:", reply_markup=reply_markup)
...
def minta_mo_numbers(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("MO Numbers tercatat.\n\nMasukkan tanggal mulai (format: YYYY-MM-DD):")
    return MINTA_TGL_MULAI
...
def minta_agency(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Masukkan link Attachment:")
...
def minta_attachments(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Attachment tercatat.\n\nMasukkan CC Email:")
...
def minta_cc_email(update: Update, context: CallbackContext) -> int:
    ...
    keyboard = [
        [
            ...
            InlineKeyboardButton("Enggak, tidak setuju", callback_data="legal_tidak")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text(
...
def tombol_konfirmasi(update: Update, context: CallbackContext) -> int:
    ...
    if query.data == "konfirm_yes":
        ...
        cc_email_str = ", ".join(cc_email_list) if isinstance(cc_email_list, list) else cc_email_list
        ...
        try:
            ...
            if result.get("success"):
                ...
                query.edit_message_text(
                    text="Error saat ngirim request: " + result.get("message", "Error nggak diketahui")
                )
        except Exception as e:
            query.edit_message_text(text="Error saat ngirim request: " + str(e))
        context.user_data.clear()
        return ConversationHandler.END
    elif query.data == "konfirm_no":
        query.edit_message_text(text="Request dibatalkan. Jika ingin ulang, ketik /start atau /reset.")
        context.user_data.clear()
        return ConversationHandler.END
...
def reset(update: Update, context: CallbackContext) -> int:
    update.message.reply_text("Request telah direset. Silakan mulai kembali dengan /start.")
    context.user_data.clear()
    return ConversationHandler.END
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            MINTA_EMAIL: [MessageHandler(Filters.text & ~Filters.command, minta_email)],
            MINTA_BRIEF: [MessageHandler(Filters.text & ~Filters.command, minta_brief)],
            PILIH_URGENSI: [CallbackQueryHandler(tombol_urgensi, pattern="^urgensi_")],
            PILIH_REQUEST_TYPE: [CallbackQueryHandler(tombol_request_type, pattern="^reqtype_")],
            PILIH_CONTENT: [CallbackQueryHandler(tombol_content, pattern="^content_")],
            MINTA_MO_NUMBERS: [MessageHandler(Filters.text & ~Filters.command, minta_mo_numbers)],
            MINTA_TGL_MULAI: [MessageHandler(Filters.text & ~Filters.command, minta_tgl_mulai)],
            MINTA_TGL_SELESAI: [MessageHandler(Filters.text & ~Filters.command, minta_tgl_selesai)],
            MINTA_BRAND: [MessageHandler(Filters.text & ~Filters.command, minta_brand)],
            MINTA_AGENCY: [MessageHandler(Filters.text & ~Filters.command, minta_agency)],
            MINTA_ATTACHMENTS: [MessageHandler(Filters.text & ~Filters.command, minta_attachments)],
            MINTA_CC_EMAIL: [MessageHandler(Filters.text & ~Filters.command, minta_cc_email)],
            PILIH_LEGAL_CHECK: [CallbackQueryHandler(tombol_legal_check, pattern="^legal_")],
            KONFIRMASI: [CallbackQueryHandler(tombol_konfirmasi, pattern="^konfirm_")],
        },
        fallbacks=[
            CommandHandler("cancel", batal),
